    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <header className="relative z-10 border-b border-slate-900 bg-slate-950/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href="/dashboard/customer" className="flex items-center gap-2 text-sm font-semibold text-slate-400 transition-colors hover:text-white">
      ...
      <main className="relative z-10 mx-auto grid max-w-5xl grid-cols-1 gap-8 px-4 py-10 sm:px-6 lg:grid-cols-12 lg:px-8">
        <section className="lg:col-span-7 space-y-6">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/40 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
            <div className="mb-6 flex items-start gap-4">
              ...
              <div>
                <h1 className="text-2xl font-black tracking-tight text-white sm:text-3xl">Invoice Pembayaran QRIS</h1>
                <p className="mt-1 text-sm leading-relaxed text-slate-400">Scan QRIS demo atau klik simulasi sukses untuk kebutuhan presentasi.</p>
              </div>
            </div>
            ...
            <div className="space-y-4 rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-2 text-sm text-slate-400"><Compass className="h-4 w-4 text-emerald-400" />Destinasi</span>
                <span className="text-right text-sm font-bold text-white">{booking.wisata?.nama_wisata || 'Wisata'}</span>
              </div>
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-2 text-sm text-slate-400"><Calendar className="h-4 w-4 text-emerald-400" />Tanggal</span>
                <span className="text-right text-sm font-bold text-white">{new Date(booking.tanggal_kunjungan).toLocaleDateString('id-ID', { dateStyle: 'long' })}</span>
              </div>
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-2 text-sm text-slate-400"><UserRound className="h-4 w-4 text-emerald-400" />Tour Guide</span>
                <span className="text-right text-sm font-bold text-white">{booking.guides?.users?.nama_lengkap || 'Tanpa Guide'}</span>
              </div>
            </div>

            <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/40 p-5 space-y-3">
              ...
              <div className="flex justify-between border-t border-slate-800 pt-4 text-lg font-black text-white">
                <span>Total Pembayaran</span>
                <span className="text-emerald-400">{formatRupiah(booking.total_harga)}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <aside className="lg:col-span-5">
          <div className="sticky top-8 rounded-3xl border border-slate-800 bg-slate-900/60 p-6 text-center shadow-2xl shadow-black/40 backdrop-blur-xl">
            ...
            <div className="mb-5 space-y-1">
              ...
              <p className="text-xs leading-relaxed text-slate-500">Kode QR dibuat dinamis dari ID booking dan total transaksi.</p>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-[11px] leading-relaxed text-emerald-200">
