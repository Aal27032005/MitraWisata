    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 relative space-y-8">
        <section className="space-y-5">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div className="space-y-3 max-w-3xl">
              ...
              <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight text-white leading-tight">{wisata.nama_wisata}</h1>
            ...
            <div className="grid grid-cols-2 gap-3 sm:min-w-80">
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4 backdrop-blur-xl">
                ...
                <strong className="mt-1 block text-lg text-white">{wisata.kuota_harian} pax</strong>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 rounded-[1.75rem] border border-slate-800/80 bg-slate-900/30 p-3 shadow-2xl shadow-black/30 backdrop-blur-xl overflow-hidden">
            {previewGallery.length > 0 ? (
              previewGallery.map((photoUrl, index) => (
                <button
                  ...
                  className={`${index === 0 ? 'col-span-2 lg:col-span-2 lg:row-span-2 h-72 sm:h-96 lg:h-[30rem]' : 'h-36 sm:h-44 lg:h-[14.625rem]'} group relative overflow-hidden rounded-2xl bg-slate-950 border border-slate-800 cursor-zoom-in`}
                >
              ...
              <div className="col-span-full h-72 md:h-96 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col items-center justify-center text-slate-700">
        ...
        <section className="lg:col-span-7 space-y-6">
          <div className="bg-slate-900/35 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
            <div className="space-y-3">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <Compass className="w-5 h-5 text-emerald-400" />
                Tentang Destinasi
              </h2>
              <p className="text-slate-300 text-sm sm:text-base leading-7 whitespace-pre-line">
                {wisata.deskripsi}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-2xl bg-slate-950/50 border border-slate-800 p-4">
                ...
                <p className="mt-1 text-sm font-semibold text-white leading-relaxed">{lokasi}</p>
              </div>
              <div className="rounded-2xl bg-slate-950/50 border border-slate-800 p-4">
                ...
                <p className="mt-1 text-sm font-semibold text-white">{wisata.kuota_harian} pengunjung</p>
              </div>
              <div className="rounded-2xl bg-slate-950/50 border border-slate-800 p-4">
        ...
        <section className="lg:col-span-5">
          <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-6 shadow-2xl shadow-black/30 space-y-6 sticky top-24">
            <div>
              <h2 className="text-lg font-bold text-white">Form Pemesanan Terpadu</h2>
            ...
            <form action={formAction} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-lg py-2.5 pl-10 pr-4 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 transition-all duration-200"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-lg py-2.5 pl-10 pr-4 text-sm text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 transition-all duration-200"
                  />
                </div>
              </div>
              ...
              {/* Rincian Total Harga */}
              <div className="bg-slate-950/40 border border-slate-850 p-4 rounded-xl space-y-2 mt-4">
                ...
                <div className="flex justify-between border-t border-slate-850 pt-3 text-sm font-bold text-white">
      ...
      {activeLightboxPhoto && (
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <button type="button" onClick={closeLightbox} className="absolute top-4 right-4 rounded-full border border-white/10 bg-white/10 p-2 text-white hover:bg-white/20 transition-colors cursor-pointer" aria-label="Tutup galeri">
            <X className="w-6 h-6" />
          </button>
          {gallery.length > 1 && (
            <button type="button" onClick={showPreviousPhoto} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full border border-white/10 bg-white/10 p-3 text-white hover:bg-white/20 transition-colors cursor-pointer" aria-label="Foto sebelumnya">
              <ChevronLeft className="w-7 h-7" />
            </button>
          )}
          <img src={activeLightboxPhoto} alt={`${wisata.nama_wisata} tampilan besar`} className="max-h-[82vh] max-w-[92vw] rounded-3xl object-contain shadow-2xl border border-white/10" />
          {gallery.length > 1 && (
            <button type="button" onClick={showNextPhoto} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full border border-white/10 bg-white/10 p-3 text-white hover:bg-white/20 transition-colors cursor-pointer" aria-label="Foto berikutnya">
              <ChevronRight className="w-7 h-7" />
            </button>
          )}
          <div className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs font-bold text-white backdrop-blur-md">
